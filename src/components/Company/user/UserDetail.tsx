...
import { editPassword } from "@/app/actions/user/editPassword";
...
// Validation schema for user form (edit mode - password and avatar are optional)
...
type PasswordErrors = Partial<
  Record<keyof z.infer<typeof passwordFormSchema> | "path", string>
>;
...
export default function UserDetail({ user }: UserDetailProps) {
  ...
  // Load avatar URL using Supabase client
  useEffect(() => {
    if (user.avatar_url) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Call editUser action
      const response = await editUser(user.user_id, formData);
      if (response.success) {
        toast.success("User updated successfully");

        // If a new avatar was uploaded, refresh the preview with cache-busting
        if (avatarFile) {
          const { data } = supabase.storage
            .from("avatars")
            .getPublicUrl(
              `${user.user_id}/avatar.${avatarFile.name.split(".").pop()}`
            );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Redirect to company-users page after successful deletion
    ...
    try {
      ...
      if (!validationResult.success) {
        ...
        validationResult.error.issues.forEach((err: any) => {
          if (err.path[0]) {
            fieldErrors[err.path[0] as keyof PasswordErrors] = err.message;
          } else {
            // For refine errors that don't have a specific path
            fieldErrors.confirmationPassword = err.message;
          }
        });
      ...
      const response = await editPassword(user.user_id, newPassword);
          ...
          <div className="bg-white p-6 border border-gray-200">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-gray-800 mb-6">
                Detail of {user.full_name}
              </h2>
              <Button
                variant="ghost"
                onClick={() => setShowPasswordDialog(true)}
                asChild
                className="bg-notice hover:bg-notice/80 text-white py-4 px-8 hover:text-white rounded-none"
              >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong>&quot;{user.full_name}&quot;</strong>? This action cannot
              be undone and will permanently remove this job posting.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <Dialog open={showPasswordDialog} onOpenChange={setShowPasswordDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter a new password for {user.full_name}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-3 pr-12 border ${
                    passwordErrors.confirmationPassword
                      ? "border-red-500"
                      : "border-gray-300"
                  } focus:outline-none focus:ring-2 focus:ring-notice`}
                />
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isChangingPassword}
              className="bg-notice text-white hover:bg-notice/80 rounded-none"
            >
