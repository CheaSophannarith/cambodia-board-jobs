...
export default function UserDetail({ userProfile }: UserDetailProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await updateUserProfile(userProfile.user_id, formData);
      if (response.success) {
        ...
        if (avatarFile) {
          const { data } = supabase.storage
            .from("avatars")
            .getPublicUrl(
              `${userProfile.user_id}/avatar.${avatarFile.name
                .split(".")
                .pop()}`
            );
          ...
          <div className="bg-white p-6 border border-gray-200">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 pr-12 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-notice"
                  />
                  <button
                    type="button"
            ...
            <div className="flex justify-end gap-4 mt-6">
              <Button
                type="button"
                variant="outline"
                className="border-red-700 text-red-700 bg-white hover:bg-slate-50 hover:text-red-700 px-6 py-3 rounded-none font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                onClick={logout}
              >
                Log Out
              </Button>
